...
const DashboardHome: FC<DashboardHomeProps> = () => {
    ...
    const moveData = async () => {
        let tempList: any[] = []
        const teamsList = await db.collection('years').doc('2025').collection("regionals").doc('cacg').collection("teams").get().then((teams)=>{
        ...
        tempList.forEach((team)=>{
            db.collection("years").doc("2025").collection("regionals").doc("cacg").collection("teams").doc(team).collection("matches").get().then((ids)=>{
                ...
                matchList.forEach((match)=>{
                    const id = match.id;
                    db.collection("years").doc('2025').collection("regionals").doc("cacg").collection("teams").doc(team).collection("matches").doc(id).get().then((data)=>{
