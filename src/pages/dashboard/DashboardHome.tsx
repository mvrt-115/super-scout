...
const DashboardHome: FC<DashboardHomeProps> = () => {
    ...
    const moveData = async () => {
        let tempList: any[] = []
        const teamsList = await db.collection('years').doc('2025').collection("regionals").doc('idbo').collection("teams").get().then((teams)=>{
        ...
        tempList.forEach((team)=>{
            db.collection("years").doc("2025").collection("regionals").doc("idbo").collection("teams").doc(team).collection("matches").get().then((ids)=>{
                const matchList = ids.docs;
                matchList.forEach((match)=>{
                    const id = match.id;
                    db.collection("years").doc('2025').collection("regionals").doc("idbo").collection("teams").doc(team).collection("matches").doc(id).get().then((data)=>{
                        ...
                        if(team=="115"){
                            console.log(temp2);}
                        db.collection("years").doc("2025").collection("regionals").doc("idbo").collection("teams").doc(team).collection("matches").doc(id).set(temp2);
                    })
                })
            })
        })
